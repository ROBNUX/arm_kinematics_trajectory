#!/usr/bin/env python3
"""
Launch KUKA KR-like POE 6-DOF arm in RViz2 (SphericalTwoParallel topology).

Usage:
    ros2 launch robnux_arm_sim poe_kuka_rviz.launch.py
    ros2 launch robnux_arm_sim poe_kuka_rviz.launch.py use_gui:=true
    ros2 launch robnux_arm_sim poe_kuka_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim poe_kuka_rviz.launch.py use_sim:=true plot:=true
    ros2 launch robnux_arm_sim poe_kuka_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    xacro_path = os.path.join(pkg, "urdf", "poe_kuka_robot.urdf.xacro")
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/poe_kuka_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_poe_kuka.py"],
            output="screen",
