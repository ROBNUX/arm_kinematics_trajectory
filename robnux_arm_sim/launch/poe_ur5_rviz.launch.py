#!/usr/bin/env python3
"""
Launch UR5-like POE 6-DOF arm in RViz2.

Usage:
    ros2 launch robnux_arm_sim poe_ur5_rviz.launch.py
    ros2 launch robnux_arm_sim poe_ur5_rviz.launch.py use_gui:=true
    ros2 launch robnux_arm_sim poe_ur5_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim poe_ur5_rviz.launch.py use_sim:=true plot:=true
    ros2 launch robnux_arm_sim poe_ur5_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    xacro_path = os.path.join(pkg, "urdf", "poe_ur5_robot.urdf.xacro")
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/poe_ur5_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_poe_ur5.py"],
            output="screen",
