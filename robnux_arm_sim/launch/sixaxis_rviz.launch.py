...
Usage:
    ...
    ros2 launch robnux_arm_sim sixaxis_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim sixaxis_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/sixaxis_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_sixaxis.py"],
            output="screen",
