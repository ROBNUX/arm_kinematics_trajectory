#!/usr/bin/env python3
"""
Launch Lite6-like compact POE 6-DOF arm in RViz2 (SphericalTwoIntersecting topology).

Usage:
    ros2 launch robnux_arm_sim poe_lite6_rviz.launch.py
    ros2 launch robnux_arm_sim poe_lite6_rviz.launch.py use_gui:=true
    ros2 launch robnux_arm_sim poe_lite6_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim poe_lite6_rviz.launch.py use_sim:=true plot:=true
    ros2 launch robnux_arm_sim poe_lite6_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    xacro_path = os.path.join(pkg, "urdf", "poe_lite6_robot.urdf.xacro")
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/poe_lite6_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "use_sim",
            default_value="false",
            description="Suppress joint_state_publisher when simulation provides /joint_states",
        ),
        DeclareLaunchArgument(
            "plot",
            default_value="false",
            description="Launch PlotJuggler for Cartesian position/orientation time-series plots",
        ),
        DeclareLaunchArgument(
            "record",
            default_value="false",
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_poe_lite6.py"],
            output="screen",
