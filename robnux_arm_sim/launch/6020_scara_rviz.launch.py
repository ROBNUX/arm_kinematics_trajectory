#!/usr/bin/env python3
"""
Launch the 6020 SCARA robot (robot_model/6020_scara.txt) in RViz2.

Usage (from a sourced ROS2 workspace):
    ros2 launch robnux_arm_sim 6020_scara_rviz.launch.py
    ros2 launch robnux_arm_sim 6020_scara_rviz.launch.py use_gui:=true
    ros2 launch robnux_arm_sim 6020_scara_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim 6020_scara_rviz.launch.py use_sim:=true plot:=true
    ros2 launch robnux_arm_sim 6020_scara_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    xacro_path = os.path.join(pkg, "urdf", "6020_scara_robot.urdf.xacro")
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/6020_scara_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "plot",
            default_value="false",
            description="Launch PlotJuggler for Cartesian position and orientation time-series plots",
        ),
        DeclareLaunchArgument(
            "record",
            default_value="false",
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_6020_scara.py"],
            output="screen",
