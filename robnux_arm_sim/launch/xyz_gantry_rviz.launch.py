#!/usr/bin/env python3
"""
Launch XYZ Gantry 3-DOF Cartesian robot in RViz2.

Usage:
    ros2 launch robnux_arm_sim xyz_gantry_rviz.launch.py
    ros2 launch robnux_arm_sim xyz_gantry_rviz.launch.py use_gui:=true
    ros2 launch robnux_arm_sim xyz_gantry_rviz.launch.py use_sim:=true
    ros2 launch robnux_arm_sim xyz_gantry_rviz.launch.py use_sim:=true record:=true
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_bag_path = os.path.expanduser(
        f"~/sim_bags/xyz_gantry_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=["ros2", "run", "robnux_arm_sim", "simulate_xyz_gantry.py"],
            output="screen",
